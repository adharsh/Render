...
namespace ginkgo {
	...
	public:
		...
		Material(const glm::vec4& color, float specularIntensity, float specularExponent, float refractiveIndex, float rIntensity, const Texture* texture);
		...
		const float getRefractiveIndex() const { return this->refractiveIndex; }

		const void setRIntensity(float rCoefficient) { this->rIntensity = rIntensity; }
		const float getRIntensity() const { return this->rIntensity; }

		const Texture* const getTexture() const { return texture; }
		const glm::vec4& getColor() const { return color; }
		const bool hasTexture() const { return texture != nullptr; }
