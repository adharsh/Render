...
namespace ginkgo {
	...
	public:
		Mesh();
		void addData(std::vector<glm::vec3>& vertices, std::vector<GLuint>& indices, std::vector<glm::vec2>& uvs, bool normals = false);
		void draw();
	private:
		...
		std::vector<glm::vec3>& calcNormals(std::vector<glm::vec3>& vertices, std::vector<GLuint>& indices);
	};
