...
namespace ginkgo {
		...
		: window(win)
	{
		...
		//when creating Renderable, pass in 2.0f as refractive index in LensLayer
		lensLayer = new LensLayer({ new Renderable(lensMesh, new Material(new Texture("Render/res/textures/Normals.bmp"))) });
		//lensLayer->alterRenderable(0)->alterModel().scaleMatrix(glm::vec3(3.0f, 3.0f, 3.0f));
	...
	//	lensLayer->alterRenderable(0)->alterModel().translateMatrix(glm::vec3(0.0f, 0.0f, -1.0f));
	//	lensLayer->alterRenderable(0)->alterModel().rotateMatrix(glm::radians(90.0f), glm::vec3(1.0f, 0.0f, 0.0f));
